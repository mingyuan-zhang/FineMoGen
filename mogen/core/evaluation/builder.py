...
EVALUATORS.register_module(name='Matching Score',
                           module=MatchingScoreEvaluator)
...
def build_evaluator(metric, eval_cfg, data_len, eval_indexes):
    ...
    if evaluator.append_indexes is not None:
        for i in range(eval_cfg['replication_times']):
            eval_indexes[i] = np.concatenate(
                (eval_indexes[i], evaluator.append_indexes[i]), axis=0)
