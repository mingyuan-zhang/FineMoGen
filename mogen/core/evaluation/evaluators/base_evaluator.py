...
class BaseEvaluator(object):
    ...
    def evaluate(self, results):
        ...
        for replication_idx in range(self.replication_times):
            partial_results = results[replication_idx *
                                      partial_len:(replication_idx + 1) *
                                      partial_len]
            if self.batch_size is not None:
                batch_metrics = []
                begin_idx = self.eval_begin_idx
                end_idx = self.eval_end_idx
                batch_size = self.batch_size
                for batch_start in range(begin_idx, end_idx, batch_size):
                    batch_results = \
                        partial_results[batch_start: batch_start + batch_size]
                ...
                batch_results = \
                    partial_results[self.eval_begin_idx: self.eval_end_idx]
    ...
    def prepare_results(self, results):
        ...
        for result in results:
            ...
            if cur_motion.shape[0] < T:
                padding_values = torch.zeros(T - cur_motion.shape[0],
                                             cur_motion.shape[1])
            ...
            if cur_pred_motion.shape[0] < T:
                padding_values = torch.zeros(T - cur_pred_motion.shape[0],
                                             cur_pred_motion.shape[1])
                padding_values = padding_values.type_as(cur_pred_motion)
                cur_pred_motion = torch.cat([cur_pred_motion, padding_values],
                                            dim=0)
            ...
            if cur_motion_mask.shape[0] < T:
                ...
                cur_motion_mask = torch.cat([cur_motion_mask, padding_values],
                                            dim=0)
            ...
            if cur_pred_motion_mask.shape[0] < T:
                ...
                cur_pred_motion_mask = torch.cat(
                    [cur_pred_motion_mask, padding_values], dim=0)
        ...
        pred_motion_length = \
            torch.Tensor(pred_motion_length).to(motion.device).long()
        output = {
