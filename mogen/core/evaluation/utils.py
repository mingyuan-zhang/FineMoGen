...
def calculate_diversity(activation, diversity_times, emb_scale, norm_scale):
    ...
    second_indices = np.random.choice(num_samples,
                                      diversity_times,
                                      replace=False)
    delta = activation[first_indices] - activation[second_indices]
    dist = linalg.norm(delta * norm_scale, axis=1)
    return dist.mean()
...
def calculate_multimodality(activation, multimodality_times):
    ...
    second_dices = np.random.choice(num_per_sent,
                                    multimodality_times,
                                    replace=False)
    delta = activation[:, first_dices] - activation[:, second_dices]
    dist = linalg.norm(delta, axis=2)
    return dist.mean()
