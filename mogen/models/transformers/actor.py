    ...
    def __init__(self,
        ...
        if self.use_condition:
            if num_class is None:
                ...
                if self.output_var:
                    self.sigma_layer = build_MLP(self.condition_dim,
                                                 self.latent_dim)
            else:
                self.mu_layer = nn.Parameter(
                    torch.randn(num_class, self.latent_dim))
                if self.output_var:
                    self.sigma_layer = nn.Parameter(
                        torch.randn(num_class, self.latent_dim))
        else:
            if self.output_var:
        ...
        if pos_embedding == 'sinusoidal':
            ...
            self.pos_encoder = LearnedPositionalEncoding(latent_dim,
                                                         dropout,
                                                         max_len=max_seq_len +
                                                         2)
    ...
    def forward(self, motion, motion_mask=None, condition=None):
        ...
        if self.output_var:
            motion_mask = torch.cat(
                (torch.zeros(B, 2).to(motion.device), 1 - motion_mask),
                dim=1).bool()
        else:
            motion_mask = torch.cat(
                (torch.zeros(B, 1).to(motion.device), 1 - motion_mask),
                dim=1).bool()
        feature = feature.permute(1, 0, 2).contiguous()
        feature = self.pos_encoder(feature)
        feature = self.seqTransEncoder(feature,
                                       src_key_padding_mask=motion_mask)
...
class ACTORDecoder(BaseModule):
        ...
        if self.use_condition:
            if num_class is None:
                ...
                self.condition_bias = nn.Parameter(
                    torch.randn(num_class, latent_dim))
        if pos_embedding == 'sinusoidal':
            self.pos_encoder = SinusoidalPositionalEncoding(
                latent_dim, dropout)
        else:
    ...
    def forward(self, input, motion_mask=None, condition=None):
        ...
        feature = self.seqTransDecoder(
            tgt=query,
            memory=input,
            tgt_key_padding_mask=(1 - motion_mask).bool())
